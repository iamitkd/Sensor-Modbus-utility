...
def samtime():
    ID = txt21.get().zfill(2)
    newtime = str(hex((int(txt4.get()))*4)).upper()
    print((int(txt4.get()))*4)
    #print(newtime)
...
def getmultipledata():
    ...
    ID1 = int(txt7.get().zfill(2))

    if txt7.index("end") == 0:
        print("Enter proper Sensor ID")
        messagebox.showinfo('Error', 'Please enter Proper sensor ID')
    if txt7_1.index("end") == 0:
        ID2 = int(txt7.get().zfill(2))
        for a in range(ID1, ID2 + 1):
            sensor_name.append(str(a))
        print(sensor_name)
    else:
        ID2 = int(txt7_1.get().zfill(2))
        for a in range(ID1, ID2 + 1):
            sensor_name.append(str(a))
        print(sensor_name)
    ...
    print(filepath)
    directory = datetime.now().strftime('%d-%m-%y-%H.%M.%S')
    ...
    SID = "SID"
    fileN = "SID"
    SensorList = [SID] * (ID2-ID1+1)
    fileName = [fileN]*(ID2-ID1+1)

    v_idx = 0
    for idx in range(ID1, ID2+1):
        SensorList[v_idx] = SensorList[v_idx]+str(idx)
        fileName[v_idx] = fileName[v_idx]+str(idx)
        v_idx += 1
    print(SensorList)
    print(fileName)
    #CsvFname = fileName[idx-1] + " " + datetime.now().strftime('%H.%M.%S')
    #path1 = os.path.join(path, CsvFname)

    file = {fileName[0]: "" }

    v_idx = 0
    final = {SensorList[v_idx]: {"Volt": [0] * sample_no, "Temp": [0] * sample_no, "C_stat": [""] * sample_no,
                             "Prev_stat": [""] * sample_no,
                             "Zmean": [0] * sample_no, "Zstd": [0] * sample_no, "Zrstd": [0] * sample_no,
                             "Zmax": [0] * sample_no,
                             "Ymean": [0] * sample_no, "Ystd": [0] * sample_no, "Yrstd": [0] * sample_no,
                             "Ymax": [0] * sample_no, "Xmean": [0] * sample_no, "Xstd":
                                 [0] * sample_no, "Xrstd": [0] * sample_no, "Xmax": [0] * sample_no}}
    for idx in range(ID1, ID2 + 1):
        final[SensorList[v_idx]] = {"Volt": [0] * sample_no, "Temp": [0] * sample_no, "C_stat": [""] * sample_no,
                             "Prev_stat": [""] * sample_no,
                             "Zmean": [0] * sample_no, "Zstd": [0] * sample_no, "Zrstd": [0] * sample_no,
                             ...
                             "Ymax": [0] * sample_no, "Xmean": [0] * sample_no, "Xstd":
                                 [0] * sample_no, "Xrstd": [0] * sample_no, "Xmax": [0] * sample_no}
        CsvFname = fileName[v_idx] + " " + datetime.now().strftime('%H.%M.%S.csv')
        path1 = os.path.join(path, CsvFname)
        file[fileName[v_idx]] = open(path1, "w+", newline='')
        writer = csv.writer(file[fileName[v_idx]], delimiter=",")
    ...
    #for b in range(0,len(sensor_name)+1):
    response = {}

    for i in range(0, sample_no):
            for j in range(ID1, ID2 + 1):
                a = makeByteBuff(readmultiple((format(j, '02X'))))
                progress['value'] = 0
                window.update_idletasks()
                ...
                if len(recvdata) >= 40:
                    ...
                    if recvdata == lastresult:
                        response[j] = (recvdata[8:(len(recvdata) - 5)])
                        #response.append(recvdata[8:(len(recvdata) - 5)])
                        # split dictionary into keys and values
                        print(response[j])
                        #for k in range(0, len(response[j])):

                        z = splitString(response[j])
                        ...
                        final[SID+str(j)]['Volt'][i] = (str(round((float)(int(z[0],16)*0x100+int(z[1],16))*(1047 / 47) * (1.114 / 1024),2)))
                        final[SID+str(j)]['Temp'][i] = (int(z[reg_temperature_Idx*2] + z[reg_temperature_Idx*2+1], 16))
                        final[SID+str(j)]['C_stat'][i] = (z[reg_Status_Idx*2] + z[reg_Status_Idx*2+1])
                        final[SID+str(j)]['Prev_stat'][i] = (z[prev_reg_Status_Idx*2] + z[prev_reg_Status_Idx*2+1])

                        final[SID+str(j)]['Zmean'][i] = (int(z[z_reg_avg_Idx*2] + z[z_reg_avg_Idx*2+1], 16))
                        final[SID+str(j)]['Zstd'][i] = (struct.unpack('!f', bytes.fromhex(z[z_reg_stddr_MSB_Idx*2] + z[z_reg_stddr_MSB_Idx*2+1] + z[z_reg_stddr_LSB_Idx*2] + z[z_reg_stddr_LSB_Idx*2+1]))[0])
                        final[SID+str(j)]['Zrstd'][i] = (struct.unpack('!f', bytes.fromhex(z[z_reg_rstddr_MSB_Idx*2] + z[z_reg_rstddr_MSB_Idx*2+1] + z[z_reg_rstddr_LSB_Idx*2] + z[z_reg_rstddr_LSB_Idx*2+1]))[0])
                        final[SID+str(j)]['Zmax'][i] = (int(z[z_reg_maxRate_Idx*2] + z[z_reg_maxRate_Idx*2+1], 16))

                        final[SID+str(j)]['Ymean'][i] = (int(z[y_reg_avg_Idx*2] + z[y_reg_avg_Idx*2+1], 16))
                        final[SID+str(j)]['Ystd'][i] = (struct.unpack('!f', bytes.fromhex(z[y_reg_stddr_MSB_Idx*2] + z[y_reg_stddr_MSB_Idx*2+1] + z[y_reg_stddr_LSB_Idx*2] + z[y_reg_stddr_LSB_Idx*2+1]))[0])
                        final[SID+str(j)]['Yrstd'][i] = (struct.unpack('!f', bytes.fromhex(z[y_reg_rstddr_MSB_Idx*2] + z[y_reg_rstddr_MSB_Idx*2+1] + z[y_reg_rstddr_LSB_Idx*2] + z[y_reg_rstddr_LSB_Idx*2+1]))[0])
                        final[SID+str(j)]['Ymax'][i] = (int(z[y_reg_maxRate_Idx*2] + z[y_reg_maxRate_Idx*2+1], 16))

                        final[SID+str(j)]['Xmean'][i]  = (int(z[x_reg_avg_Idx*2] + z[x_reg_avg_Idx*2+1], 16))
                        final[SID+str(j)]['Xstd'][i] = (struct.unpack('!f', bytes.fromhex(z[x_reg_stddr_MSB_Id*2] + z[x_reg_stddr_MSB_Id*2+1] + z[x_reg_stddr_LSB_Idx*2] + z[x_reg_stddr_LSB_Idx*2+1]))[0])
                        final[SID+str(j)]['Xrstd'][i] = (struct.unpack('!f', bytes.fromhex(z[x_reg_rstddr_MSB_Idx*2] + z[x_reg_rstddr_MSB_Idx*2+1] + z[x_reg_rstddr_LSB_Idx*2] + z[x_reg_rstddr_LSB_Idx*2+1]))[0])
                        final[SID+str(j)]['Xmax'][i] = (int(z[x_reg_maxRate_Idx*2] + z[x_reg_maxRate_Idx*2+1], 16))
                        time.sleep(0.002)
                time.sleep(t1)
                progress['value'] = 100
                window.update_idletasks()
    ...
    fields = ['Time', 'Voltage', 'Temperature', 'Current_Status', 'Prev_Status', 'Z-Mean Value', 'Z-Stdr',
              'Z-Rstdr', 'Z-Max Rate', 'Y-Mean Value', 'Y-Stdr', 'Y-Rstdr', 'Y-Max Rate', 'X-Mean', 'X-Stdr', 'X-Rstdr',
              'X-Max Rate']
    ...
    for i in range(sample_no):
            ...
            for j in range(ID1,ID2+1):
                writer = csv.writer(file[fileName[v_idx]], delimiter=",")
                writer.writerow([timev[i], final[SID+str(j)]['Volt'][i], final[SID+str(j)]['Temp'][i],"'" + final[SID+str(j)]['C_stat'][i],"'" + final[SID+str(j)]['Prev_stat'][i],final[SID+str(j)]['Zmean'][i], final[SID+str(j)]['Zstd'][i], final[SID+str(j)]['Zrstd'][i], final[SID+str(j)]['Zmax'][i],final[SID+str(j)]['Ymean'][i], final[SID+str(j)]['Ystd'][i], final[SID+str(j)]['Yrstd'][i], final[SID+str(j)]['Ymax'][i],final[SID+str(j)]['Xmean'][i], final[SID+str(j)]['Xstd'][i],final[SID+str(j)]['Xrstd'][i], final[SID+str(j)]['Xmax'][i]])
                v_idx += 1

    messagebox.showinfo('File Saved', 'CSV File saved to the Selected Location')

########################################################################################################################

if __name__ == "__main__":
    window = Tk()
    window.title("Sensor Modbus Utility V1.2")
...
#########################################  main window buttons ######################################################
    ...
    cb2['values'] = (230400,115200,76800,57600,38400,28800)
    ...
    lbl21.grid(column=1, row=1,pady=20)
    txt21 = Entry(tab1, width=10)
    txt21.grid(column=2, row=1)

    progress = Progressbar(tab1, orient=HORIZONTAL,
                           length=7, mode='determinate')
    progress.grid(column=2,columnspan=5, row=1, ipadx=110, pady=10)
    ...
    lbl7 = Label(tab1, text='Enter Sensor Range')
    lbl7.grid(column=6, row=7,padx=10,pady=10)
    txt7 = Entry(tab1, width=10)
    txt7.grid(column=7, row=7)
    txt7_1 = Entry(tab1, width=10)
    txt7_1.grid(column=8, row=7)
    ...
    btn9.grid(column=8, row=12,ipadx=30)
...
########################################################################################################
    ...
    inf1 = Label(tab4, text='This is a text sample where about section needs to be updated')
    inf1.grid( row=1, pady=20,padx=10)

    separator = ttk.Separator(tab4, orient='horizontal')
    separator.grid(row=2, columnspan=15, ipadx=370, pady=10)

    inf2 = Button(tab4, text="Know More", command=hlp)
    inf2.grid( row=3, padx=10)
